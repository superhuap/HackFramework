...
namespace
{
    ...
    ID3D10RenderTargetView* g_renderTarget = nullptr;
    ...
    bool CreateDeviceD3D10(HWND hWnd)
    {
        ...
        const HRESULT hr = D3D10CreateDeviceAndSwapChain(nullptr, D3D10_DRIVER_TYPE_NULL, nullptr, 0,
                                                         D3D10_SDK_VERSION, &swapChainDesc, &g_swapChain, &g_device);
        if (hr != S_OK)
        {
            LOG_ERROR("D3D10CreateDeviceAndSwapChain() failed. [rv: {}]", static_cast<unsigned long>(hr));
            return false;
        }
    ...
    void RenderImGui_DX10(IDXGISwapChain* pSwapChain)
    {
        ...
        if (ImGui::GetCurrentContext() && g_device && g_renderTarget)
        {
...
bool DX10_Backend::Initialize(HWND hWnd)
{
    ...
    LOG_INFO("DirectX10: g_pd3dDevice: {}", reinterpret_cast<void*>(g_device));
    LOG_INFO("DirectX10: g_pSwapChain: {}", reinterpret_cast<void*>(g_swapChain));
    ...
    LOG_INFO("DirectX10 Backend initialized successfully");
    return true;
}

void DX10_Backend::Shutdown()
{
    LOG_INFO("DirectX10 Backend shutting down...");
